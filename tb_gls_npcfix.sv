...
module cpu_4bit_tb_gls_npcfix;
  ...
  initial begin
    if ($test$plusargs("DUMP_WAVE")) begin
      ...
      $dumpvars(0, cpu_4bit_tb_gls_npcfix);
    end
  end
  ...
  initial begin
    ...
    if ({dut.\acc[3] , dut.\acc[2] , dut.\acc[1] , dut.\acc[0] } == 4'd6) begin
      $display("POST-LAYOUT NPCFIX GLS PASSED: ACC = 6");
    end else begin
      $error("POST-LAYOUT NPCFIX GLS FAILED: ACC = %b%b%b%b, expected 0110", dut.\acc[3] , dut.\acc[2] , dut.\acc[1] , dut.\acc[0] );
    end
  ...
  initial begin
    #2000;
    $fatal(1, "POST-LAYOUT NPCFIX GLS TIMEOUT: CPU did not halt");
  end
